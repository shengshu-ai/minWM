...
from .device_communicators.cpu_communicator import CpuCommunicator
...
class GroupCoordinator:
    ...
    def recv_object(self, src: int) -> Any:
        ...
        object_tensor = torch.empty(  # type: ignore[call-overload]
            size_tensor.item(), dtype=torch.uint8, device="cpu"  # type: ignore[arg-type]
        )
...
def get_device_mesh() -> DeviceMesh:
    """Return the global N-D ``DeviceMesh`` spanning the DP and SP dimensions.

    Built by :func:`initialize_model_parallel` whenever CUDA is available. The
    mesh shares its rank layout with the ``GroupCoordinator``-based SP/DP groups
    (SP = consecutive ranks, data-parallel = strided), so FSDP and the SP
    collectives address the same physical rank groups.

    Returns:
        DeviceMesh: named dims ``("dp_shard", "sp")`` (pure FSDP) or
        ``("dp_replicate", "dp_shard", "sp")`` (HSDP), plus a flattened
        ``"dp_shard_cp"`` dim that FSDP shards over (see :func:`get_fsdp_mesh`).

    Raises:
        AssertionError: if the mesh has not been initialized.
    """
    assert _DEVICE_MESH is not None, "device mesh is not initialized"
    return _DEVICE_MESH


def get_fsdp_mesh() -> DeviceMesh:
    """Return the submesh ``fully_shard`` shards parameters and reduces grads over.

    Following torchtitan, FSDP shards over the **flattened** ``dp_shard`` + ``sp``
    dimensions (the ``"dp_shard_cp"`` mesh dim). Including ``sp`` is required for
    *correctness*, not just memory: minwm's Ulysses SP all-gather has a
    ``reduce_scatter`` backward that scales each rank's parameter gradient by
    ``sp``, and that factor is only cancelled by FSDP's mean-reduction across the
    SP dimension. It also shards parameters over SP ranks (more memory savings).

    For HSDP the returned mesh is 2-D ``("dp_replicate", "dp_shard_cp")`` so FSDP
    reduce-scatters within ``dp_shard_cp`` and all-reduces across ``dp_replicate``.

    Returns:
        DeviceMesh: the submesh to pass to ``fully_shard``.
    """
    mesh = get_device_mesh()
    if "dp_replicate" in mesh.mesh_dim_names:
        return mesh["dp_replicate", "dp_shard_cp"]
    return mesh["dp_shard_cp"]
...
def _init_device_mesh(
    ...
    """Build the global ``DeviceMesh`` consistent with the SP/DP rank layout.

    The SP groups are consecutive ranks and the (data-parallel) groups are
    strided, which is exactly the layout ``init_device_mesh`` produces with
    ``sp`` as the fastest-varying (last) dimension. The ``dp_shard`` and ``sp``
    dims are then flattened into a single ``"dp_shard_cp"`` dim (torchtitan's
    name) that FSDP shards / reduces over, so the SP gradient sum folds into the
    FSDP reduce. A non-zero ``data_parallel_shard_size`` carves an HSDP
    ``dp_replicate`` dimension out of the data-parallel ranks.

    Args:
        world_size (int): global process count.
        sequence_model_parallel_size (int): SP degree (the ``sp`` dim size).
        data_parallel_shard_size (int): HSDP shard degree (number of ranks each
            replica is sharded across, excluding SP); ``0`` means no replicate
            dimension (pure FSDP over all ``dp_shard`` x ``sp`` ranks).
    """
    global _DEVICE_MESH
    if not torch.cuda.is_available():
        return
    dp_size = world_size // sequence_model_parallel_size
    if data_parallel_shard_size and 0 < data_parallel_shard_size < dp_size:
        assert dp_size % data_parallel_shard_size == 0, (
            f"dp size ({dp_size}) must be divisible by "
            f"data_parallel_shard_size ({data_parallel_shard_size})"
        )
        dp_replicate = dp_size // data_parallel_shard_size
        mesh = init_device_mesh(
            "cuda",
            (dp_replicate, data_parallel_shard_size, sequence_model_parallel_size),
            mesh_dim_names=("dp_replicate", "dp_shard", "sp"),
        )
    else:
        mesh = init_device_mesh(
            "cuda",
            (dp_size, sequence_model_parallel_size),
            mesh_dim_names=("dp_shard", "sp"),
        )
    # Flatten dp_shard x sp into the single dim FSDP shards / reduces over.
    mesh["dp_shard", "sp"]._flatten("dp_shard_cp")
    _DEVICE_MESH = mesh
...
def initialize_model_parallel(
    ...
    data_parallel_shard_size: int = 0,
) -> None:
    """
    ...
    for i in range(num_sequence_model_parallel_groups):
        ...
        ranks = list(
            range(i * sequence_model_parallel_size, (i + 1) * sequence_model_parallel_size)
        )
        group_ranks.append(ranks)
    ...
    _init_device_mesh(world_size, sequence_model_parallel_size, data_parallel_shard_size)
...
def maybe_init_distributed_environment_and_model_parallel(
    ...
    logger.info(
        "Initializing distributed environment with world_size=%d, device=%s", world_size, device
    )
...
def initialize_sequence_parallel_group(
    ...
    sp_group = init_model_parallel_group(
