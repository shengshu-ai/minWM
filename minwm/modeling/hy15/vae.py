...
"""Causal-cache 3D Conv KL-VAE (``AutoencoderKLConv3D``) for the HY15 pipeline."""
    ...
    def __init__(
        ...
        for i_level, ch in enumerate(block_out_channels):
            ...
            if add_spatial_downsample or add_temporal_downsample:
                ...
                block_out = (
                    block_out_channels[i_level + 1] if downsample_match_channel else block_in
                )
    ...
    def tile_parallel_spatial_tiled_decode(self, z: torch.Tensor):
        ...
        for lin_idx in my_linear_indices:
            ...
            decoded_metas.append(
                torch.tensor([ri, rj, pad_w, pad_h], device=z.device, dtype=torch.int64)
            )

        while len(decoded_tiles) < tiles_per_rank:
            T_out = (
                decoded_tiles[0].shape[2]
                if len(decoded_tiles) > 0
                else (T - 1) * self.ffactor_temporal + 1
            )
