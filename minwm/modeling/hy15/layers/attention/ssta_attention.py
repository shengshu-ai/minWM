...
"""Sparse Spatio-Temporal Attention (SSTA) block-mask builders and kernel wrapper.

Implements the tiled MOBA / STA / SSTA block-mask construction and the
``ssta_3d_attention`` dispatch used by the HY15 attention modes. The
``flex_block_attn`` kernel is imported lazily inside ``ssta_3d_attention`` so
importing this module never forces the optional dependency.
"""
...
def tile(x, canvas_thw, tile_thw, sp_size=1):
    """Rearrange a tensor into tiles for block-based attention.

    Args:
        x (torch.Tensor): input tensor ``(b, head, s, d)`` where ``s = t * h * w``.
        canvas_thw (tuple): ``(t, h, w)`` temporal/height/width dimensions.
        tile_thw (tuple): ``(tile_t, tile_h, tile_w)`` tile dimensions.
        sp_size (int): sequence-parallel size.

    Returns:
        torch.Tensor: tensor reorganized by tiles.
    """
    b, h, s, d = x.shape
    t, h, w = canvas_thw
...
def untile(x, canvas_thw, tile_thw, sp_size=1):
    """Reverse :func:`tile` to restore the original tensor layout.

    Args:
        x (torch.Tensor): tiled tensor.
        canvas_thw (tuple): ``(t, h, w)`` temporal/height/width dimensions.
        tile_thw (tuple): ``(tile_t, tile_h, tile_w)`` tile dimensions.
        sp_size (int): sequence-parallel size.

    Returns:
        torch.Tensor: tensor with the original layout restored.
    """
    t, h, w = canvas_thw
...
def importance_sampling(q, k, topk, threshold=0.0, lambda_=0.9, adaptive_pool=None):
    """Select top-k blocks by importance (similarity minus redundancy).

    Args:
        q (torch.Tensor): query tensor ``(B, H, S, D)``.
        k (torch.Tensor): key tensor ``(B, H, K, D)``.
        topk (int): number of top blocks to select.
        threshold (float): threshold parameter (not implemented).
        lambda_ (float): weight balancing similarity and redundancy.
        adaptive_pool: adaptive pooling parameter (unused).

    Returns:
        torch.Tensor: indices of selected blocks ``(B, H, S, topk)``.

    Raises:
        NotImplementedError: if ``threshold`` is positive.
    """
    if threshold > 0.0:
...
def similarity_sampling(
    ...
    """Select top-k blocks by query/key similarity scores.

    Args:
        q (torch.Tensor): query tensor ``(B, H, S, D)``.
        k (torch.Tensor): key tensor ``(B, H, K, D)``.
        topk (int): number of top blocks to select.
        threshold (float): cumulative-score threshold for dynamic topk.
        block_num (int, optional): total number of blocks (unused).
        adaptive_pool: adaptive pooling parameter (unused).
    ...
    if threshold > 0.0:
        ...
        top_block_indices = (
            torch.gather(sorted_indices, -1, indices) * mask + (1 - mask) * sorted_indices[..., 0:1]
        )
    else:
        gate = torch.einsum("bhsd,bhkd->bhsk", q, k)
...
def create_moba_3d_mask(
    ...
    """Create a MOBA (Mixture of Block Attention) 3D mask for sparse attention.

    Args:
        q (torch.Tensor): query tensor.
        k (torch.Tensor): key tensor.
        canvas_thw (tuple): canvas dimensions ``(t, h, w)``.
        topk (int): number of top blocks to attend to.
        tile_thw (tuple): tile dimensions.
        kernel_thw (tuple): kernel dimensions.
        text_block_num (int): number of text blocks.
        add_text_mask (bool): whether to add a text mask.
        threshold (float): threshold for similarity sampling.
        lambda_ (float, optional): weight factor for importance sampling.
        mask_share_within_head (bool): whether to share the mask across heads.
        q_block_avg_pool (bool): whether to average-pool query blocks.
        adaptive_pool: adaptive pooling size.
        sampling_type (str): ``"similarity"`` or ``"importance"``.

    Returns:
        torch.Tensor: 3D attention mask ``(num_heads, block_num, block_num)``.

    Raises:
        NotImplementedError: if ``sampling_type`` is unsupported.
    """
    seq_len = q.size(2)
    ...
    def get_block_avg_feat(x, adaptive_pool=None, pooling_type="avg"):
        ...
        if adaptive_pool is not None:
            x_block_means = x_block_means.view(
                -1, block_shape[0], block_shape[1], block_shape[2], head_dim
            )
            ...
            x_block_means = x_block_means.mean(dim=(-2, -3, -4)).view(
    ...
    k_block_means = k_block_means.type(torch.float32)
    ...
    top_block_indices = top_block_indices.squeeze(0)
...
def create_sta_3d_mask_optimize(canvas_thw, tile_thw, kernel_thw):
    """Create an optimized STA 3D mask using vectorized operations.

    Args:
        canvas_thw (str): canvas dimensions ``"t_h_w"``.
        tile_thw (str): tile dimensions ``"t_h_w"``.
        kernel_thw (str): kernel dimensions ``"t_h_w"``.

    Returns:
        torch.Tensor: boolean mask ``(block_num, block_num)``.
    """
    ...
    tile_thw_num = (
        canvas_thw[0] // tile_thw[0],
        canvas_thw[1] // tile_thw[1],
        canvas_thw[2] // tile_thw[2],
    )
...
def create_sta_3d_mask(canvas_thw, tile_thw, kernel_thw, text_block_num=0):
    """Create an STA (Spatio-Temporal Attention) 3D mask.

    Args:
        canvas_thw (tuple): canvas dimensions ``(t, h, w)``.
        tile_thw (tuple): tile dimensions.
        kernel_thw (tuple): kernel dimensions.
        text_block_num (int): number of text blocks to pad.

    Returns:
        torch.Tensor: boolean mask with optional text-block padding.
    """
    block_mask = create_sta_3d_mask_optimize(
        "_".join([str(x) for x in canvas_thw]),
    ...
    if text_block_num > 0:
        pad_block_num = block_num + text_block_num
        sta_mask = torch.full((pad_block_num, pad_block_num), False, dtype=torch.bool)
...
def create_ssta_3d_mask(
    ...
    """Create an SSTA 3D mask combining STA and MOBA masks.
    ...
    Args:
        ...
        threshold (float): threshold for similarity sampling.
        lambda_ (float, optional): weight factor for importance sampling.
        text_mask (torch.Tensor, optional): optional text mask tensor.
        mask_share_within_head (bool): whether to share the mask across heads.
        adaptive_pool: adaptive pooling size.
        sampling_type (str): ``"similarity"`` or ``"importance"``.

    Returns:
        torch.Tensor: combined 3D attention mask.
    """
    ...
    if text_mask is not None:
        ...
        ssta_3d_mask[:, :, pad_start_index:] = False
        eye_mask = torch.eye(
            ssta_3d_mask.shape[1] - pad_start_index,
            dtype=torch.bool,
            device=ssta_3d_mask.device,
        ).unsqueeze(0)
        ssta_3d_mask[:, pad_start_index:, pad_start_index:] = (
            ssta_3d_mask[:, pad_start_index:, pad_start_index:] | eye_mask
        )
    return ssta_3d_mask
...
def ssta_3d_attention(
    ...
    """Sparse Spatio-Temporal Attention (SSTA) 3D attention.

    Args:
        all_q (torch.Tensor): query tensor ``(B, H, S, D)``.
        all_k (torch.Tensor): key tensor ``(B, H, S, D)``.
        all_v (torch.Tensor): value tensor ``(B, H, S, D)``.
        canvas_thw (tuple): canvas dimensions ``(t, h, w)``.
        topk (int): number of top blocks to attend to.
        tile_thw (tuple): tile dimensions.
        kernel_thw (tuple): kernel dimensions.
        text_len (int): length of the text sequence.
        sparse_type (str): ``"sta"``, ``"block_attn"``, or ``"ssta"``.
        threshold (float): threshold for similarity sampling.
        lambda_ (float, optional): weight factor for importance sampling.
        pad_type (str): ``"zero"`` or ``"repeat"``.
        text_mask (torch.Tensor, optional): optional text mask tensor.
        mask_share_within_head (bool): whether to share the mask across heads.
        sampling_type (str): ``"similarity"`` or ``"importance"``.
        adaptive_pool: adaptive pooling size.
    ...
    Returns:
        tuple[torch.Tensor, float]: ``(output ``(B, H, S, D)``, sparse_ratio)``.

    Raises:
        Exception: if ``flex_block_attn`` is unavailable or ``sparse_type`` is invalid.
    """
    try:
        from flex_block_attn import flex_block_attn_func
    except ImportError as e:
        raise Exception(
            "Could not load flex-block-attn. Please install the flex-block-attn package. "
            "You can install it via 'pip install flex-block-attn'."
        ) from e
    ...
    if sparse_type == "sta":
        ...
        block_mask = create_sta_3d_mask(canvas_thw, tile_thw, kernel_thw, text_block_num).to(
            q.device
        )
