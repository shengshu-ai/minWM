...
"""Activation-layer factory for the HY15 DiT."""

import torch.nn as nn


def get_activation_layer(act_type):
    """Resolve an activation-layer constructor by name.

    Args:
        act_type (str): one of ``"gelu"``, ``"gelu_tanh"``, ``"relu"``, ``"silu"``.

    Returns:
        Callable: a no-arg callable that constructs the activation module.

    Raises:
        ValueError: if ``act_type`` is unknown.
    """
    if act_type == "gelu":
