...
"""Norm-layer factory for the HY15 DiT.

``RMSNorm`` is the shared implementation from :mod:`minwm.modeling.common.norm`,
re-exported here so existing ``from ...norm import RMSNorm`` sites are unchanged.
"""
...
__all__ = ["RMSNorm", "get_norm_layer"]


def get_norm_layer(norm_layer):
    """Resolve a norm-layer constructor by name.

    Args:
        norm_layer (str): one of ``"layer"`` or ``"rms"``.

    Returns:
        type: the corresponding ``nn.Module`` subclass.

    Raises:
        NotImplementedError: if ``norm_layer`` is unknown.
    """
    if norm_layer == "layer":
        return nn.LayerNorm
    elif norm_layer == "rms":
