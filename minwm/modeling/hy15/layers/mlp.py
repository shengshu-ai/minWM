...
class MLP(nn.Module):
    ...
    Args:
        ...
        hidden_channels (int, optional): hidden dimension; defaults to ``in_channels``.
        out_features (int, optional): output dimension; defaults to ``in_channels``.
        act_layer (Callable): activation module constructor.
        norm_layer (Callable, optional): optional norm applied between fc1 and fc2.
        bias (bool | tuple[bool]): per-layer bias flags.
        drop (float | tuple[float]): per-layer dropout probabilities.
        use_conv (bool): use Conv2d(kernel_size=1) instead of Linear.
...
class LinearWarpforSingle(nn.Module):
    """Concatenate two tensors along dim-2 then project."""
...
class MLPEmbedder(nn.Module):
    """Two-layer SiLU embedder (from Flux).
...
class FinalLayer(nn.Module):
    """The final adaLN + linear projection layer of the DiT.

    Args:
        hidden_size (int): model hidden dimension.
        patch_size (int | tuple): spatial patch size; used to compute the output linear width.
        out_channels (int): output channel count per patch.
