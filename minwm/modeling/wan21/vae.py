...
class Wan21VAE:
    ...
    def decode(self, zs):
        # ``minWM-dev`` inference decodes through ``Wan21VAEWrapper`` after the
        # wrapper is moved to bf16, without an autocast region. Keep encode
        # autocast for fp32 input robustness, but decode with the module/input
        # dtypes directly so pixel parity matches the legacy inference path.
